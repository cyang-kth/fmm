...
TEST_CASE("Network graph is tested", "[network_graph]")
{
    ...
    {
        REQUIRE(network.get_edge_id(
                    ng.get_edge_index(network.get_node_index(11),
                                      network.get_node_index(12), 1)) == 20);
        REQUIRE(ng.get_edge_index(network.get_node_index(11),
                                  network.get_node_index(12), 0.5) == -1);
    }
    ...
    SECTION("bidirectional_dijkstra")
    {
        ...
        std::vector<EdgeIndex> path1 =
            ng.shortest_path_dijkstra(source, target);
        std::vector<EdgeIndex> path2 =
            bng.shortest_path_bidirectional_dijkstra(source, target);
    ...
    SECTION("bidirectional_dijkstra_within_dist")
    {
        EdgeID edge_id = 2;
        double dist = 3;
        auto result = bng.search_edges_within_dist_ft_edge(edge_id, dist);
