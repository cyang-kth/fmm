...
public:
     ...
     */
    NetworkGraphOptOmp(Network *network) {
    ...
    void precompute_ubodt(const std::string &filename, double delta, bool binary=true) {
        ...
        // } else {
            ...
            for (int i=0; i<num_vertices; ++i) {
                ...
                driving_distance_csv(i, delta, myfile, thread_id);
                ++progress;
                if (progress%step_size==0) {
                    std::stringstream buf;
                    buf << "Progress " << progress << " / " << num_vertices <<'\n';
                    std::cout << buf.rdbuf();
                }
            }
            // for (boost::tie(vi, vend) = vertices(g); vi != vend; ++vi) {
...
private:
     ...
     */
    void driving_distance_csv(const vertex_descriptor& source, double delta, std::ostream& stream, int thread_id) {
        std::deque<vertex_descriptor> nodesInDistance;
        ...
        while (k < nodesInDistance.size()) {
            ...
            if (source != node) {
                ...
                stream_pool[thread_id] << vertex_id_vec[source] << ";" << vertex_id_vec[node] << ";" << vertex_id_vec[successors[k]] << ";"
        ...
        #pragma omp critical
        stream<<stream_pool[thread_id].rdbuf();
