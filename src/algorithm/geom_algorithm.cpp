...
LineString interpolate_line_distances(const LineString &line,
  ...
  while(i<Npoints-1 && j<distance_count) {
    ...
    while (j<distance_count && length_visited<=distances[j]
      ...
      // SPDLOG_INFO("Ratio is {}",ratio);
      interpolated_line.add_point(ratio*(x2-x1)+x1,ratio*(y2-y1)+y1);
      ++j;
    }
    length_visited+=temp;
    ++i;
  ...
  if (length_visited<distances.back()) {
    interpolated_line.add_point(line.get_x(Npoints-1),line.get_y(Npoints-1));
  }
...
LineString interpolate_line_distance(const LineString &line,
  ...
  // SPDLOG_INFO("Length is {} and last distance is {}", length, distances.back());
