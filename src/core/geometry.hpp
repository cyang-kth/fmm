...
class LineString
{
  public:
    ...
    {
        return Point(boost::geometry::get<0>(line.at(i)),
                     boost::geometry::get<1>(line.at(i)));
    };
    /**
     ...
     */
    inline const Point &at(int i) const { return line.at(i); }
    /**
     ...
     */
    inline bool is_empty() { return boost::geometry::num_points(line) == 0; };
    /**
     * Remove all points in the current line.
     */
    inline void clear() { boost::geometry::clear(line); };
    /**
     * Get the length of the line
     * @return the length value
     */
    inline double get_length() const { return boost::geometry::length(line); };
    /**
    ...
    inline bool operator==(const LineString &rhs) const
    {
        ...
        bool result = true;
        for (int i = 0; i < N; ++i) {
            if (boost::geometry::distance(get_point(i), rhs.get_point(i)) >
                1e-6)
                result = false;
        }
        return result;
