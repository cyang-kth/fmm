...
BidirectionalNetworkGraph::BidirectionalNetworkGraph(const Network &network_arg)
    : NetworkGraph(network_arg)
{
    ...
    NodeIndex source, NodeIndex target) const
{
    ...
    std::vector<EdgeIndex> fpath = back_track(source, prev_node, pmap, fdmap);
...
void BidirectionalNetworkGraph::single_target_upperbound_dijkstra(
    NodeIndex target, double delta, SuccessorMap *smap, DistanceMap *dmap) const
{
    Heap Q;
    // Initialization
    Q.push(target, 0);
...
void BidirectionalNetworkGraph::forward_search(Heap *Q, NodeIndex u,
                                               double dist,
                                               PredecessorMap *pmap,
                                               DistanceMap *dmap) const
{
    ...
    double temp_dist = 0;
    for (boost::tie(out_i, out_end) = boost::out_edges(u, g); out_i != out_end;
         ++out_i) {
...
void BidirectionalNetworkGraph::backward_search(Heap *Q, NodeIndex v,
                                                double dist, SuccessorMap *smap,
                                                DistanceMap *dmap) const
{
    double temp_dist = 0;
...
std::vector<EdgeIndex>
BidirectionalNetworkGraph::forward_track(NodeIndex source, NodeIndex target,
...
std::unordered_set<EdgeID>
BidirectionalNetworkGraph::search_edges_within_dist_to_node(NodeIndex target,
                                                            double delta) const
{
    std::unordered_set<EdgeID> result;
    Heap Q;
...
std::unordered_set<EdgeID>
BidirectionalNetworkGraph::search_edges_within_dist_ft_edge(EdgeID eid,
                                                            double dist) const
{
