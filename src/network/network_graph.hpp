...
class NetworkGraph
{
  public:
     ...
     * @param source
     * @param target
     * @return a vector of edge index representing the path from source to
     * target
     */
    std::vector<EdgeIndex> shortest_path_dijkstra(NodeIndex source,
                                                  NodeIndex target) const;
    /**
     * Calculate heuristic distance from p1 to p2,which is used in Astar
     * routing.
     * @param p1
     * @param p2
     * @return the Euclidean distance from p1 to p2
     */
    double calc_heuristic_dist(const CORE::Point &p1,
                               const CORE::Point &p2) const;
    /**
     ...
     */
    void single_source_upperbound_dijkstra(NodeIndex source, double delta,
                                           PredecessorMap *pmap,
