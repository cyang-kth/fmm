...
const Graph_T &NetworkGraph::get_boost_graph() const { return g; }

const Network &NetworkGraph::get_network() { return network; }
unsigned int NetworkGraph::get_num_vertices() const { return num_vertices; }

std::vector<EdgeIndex>
NetworkGraph::shortest_path_dijkstra(NodeIndex source, NodeIndex target) const
{
...
double NetworkGraph::calc_heuristic_dist(const Point &p1, const Point &p2) const
{
    return sqrt((p2.get<0>() - p1.get<0>()) * (p2.get<0>() - p1.get<0>()) +
                (p2.get<1>() - p1.get<1>()) * (p2.get<1>() - p1.get<1>()));
}

std::vector<EdgeIndex> NetworkGraph::shortest_path_astar(NodeIndex source,
                                                         NodeIndex target) const
{
    ...
    // Initialization
    double h =
        calc_heuristic_dist(vertex_points[source], vertex_points[target]);
    Q.push(source, h);
    pmap.insert({source, source});
    dmap.insert({source, 0});
    OutEdgeIterator out_i, out_end;
...
std::vector<EdgeIndex> NetworkGraph::back_track(NodeIndex source,
                                                NodeIndex target,
                                 ...
                                 double *cost) const
{
    ...
    for (boost::tie(out_i, out_end) = boost::out_edges(source, g);
         out_i != out_end; ++out_i) {
        e = *out_i;
        SPDLOG_TRACE("  Check Edge from {} to {} cost {}", source,
                     boost::target(e, g), g[e].length);
        if (target == boost::target(e, g) && g[e].length <= current_cost) {
...
void NetworkGraph::single_source_upperbound_dijkstra(NodeIndex s, double delta,
                                                     PredecessorMap *pmap,
