...
public:
  ...
  void run(){
    ...
    Network network(config.network_config.file,
                    config.network_config.id,
                    config.network_config.source,
                    config.network_config.target);
    NetworkGraph graph(network);
    FMM mm_model(network, graph, ubodt);
