...
void FastMapMatchConfig::print() const
{
    ...
    SPDLOG_INFO("k {} radius {} gps_error {} reverse_tolerance {}", k, radius,
                gps_error, reverse_tolerance);
};

FastMapMatchConfig
FastMapMatchConfig::load_from_xml(const boost::property_tree::ptree &xml_data)
{
...
FastMapMatchConfig
FastMapMatchConfig::load_from_arg(const cxxopts::ParseResult &arg_data)
{
...
std::string
FastMapMatch::match_gps_file(const FMM::CONFIG::GPSConfig &gps_config,
                             const FMM::CONFIG::ResultConfig &result_config,
                             const FastMapMatchConfig &fmm_config, bool use_omp)
{
...
void FastMapMatch::update_tg(TransitionGraph *tg, const Trajectory &traj,
                             double reverse_tolerance)
{
...
void FastMapMatch::update_layer(int level, TGLayer *la_ptr, TGLayer *lb_ptr,
                                ...
                                bool *connected)
{
    ...
    for (auto iter_a = la_ptr->begin(); iter_a != la_ptr->end(); ++iter_a) {
        ...
        for (auto iter_b = lb_ptr->begin(); iter_b != lb_ptr->end(); ++iter_b) {
            double sp_dist =
                get_sp_dist(iter_a->c, iter_b->c, reverse_tolerance);
            ...
            SPDLOG_TRACE(
                ...
                iter_a->c->edge->id, iter_b->c->edge->id, sp_dist, eu_dist, tp,
                iter_b->ep, iter_a->cumu_prob, temp);
