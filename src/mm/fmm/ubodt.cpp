...
{
    SPDLOG_TRACE("Intialization UBODT with buckets {} multiplier {}", buckets,
                 multiplier);
...
double UBODT::get_delta() const { return delta; }
...
long UBODT::estimate_ubodt_rows(const std::string &filename)
{
    ...
    if (rc == 0) {
        ...
        std::string fn_extension =
            filename.substr(filename.find_last_of(".") + 1);
        std::transform(fn_extension.begin(), fn_extension.end(),
                       fn_extension.begin(),
...
{
    std::vector<int> prime_numbers = {5003,    10039,   20029,    50047,
                                      100669,  200003,  500000,   1000039,
                                      2000083, 5000101, 10000103, 20000033};
                                             ...
                                             int multiplier)
{
    ...
    while (fgets(line, BUFFER_LINE, stream)) {
        ...
        sscanf(line, "%d;%d;%d;%d;%d;%lf", &r->source, &r->target, &r->first_n,
