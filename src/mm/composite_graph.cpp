                       ...
                       double reverse_tolerance)
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (const Candidate &c : pcs) {
            ...
            add_edge(n, c.edge->target, c.edge->index,
                     c.edge->length - c.offset);
            ...
            if (iter != prev_cmap->end()) {
                if (iter->second->offset <= c.offset) {
                    add_edge(iter->second->index, n, c.edge->index,
                             c.offset - iter->second->offset);
                } else if (iter->second->offset - c.offset <
                           c.edge->length * reverse_tolerance) {
                    add_edge(iter->second->index, n, c.edge->index, 0);
                }
            }
        }
...
Graph_T *DummyGraph::get_graph_ptr() { return &g; }

const Graph_T &DummyGraph::get_boost_graph() const { return g; }

int DummyGraph::get_num_vertices() const { return boost::num_vertices(g); }
...
int DummyGraph::get_edge_index(NodeIndex source, NodeIndex target,
                               double cost) const
{
    SPDLOG_TRACE("Dummy graph get edge index {} {} cost {}", source, target,
                 cost);
    EdgeDescriptor e;
    ...
    for (boost::tie(out_i, out_end) = boost::out_edges(source_idx, g);
         out_i != out_end; ++out_i) {
        e = *out_i;
        SPDLOG_TRACE("Target index {} {} id {} e length {} {}", target_idx,
                     boost::target(e, g), g[e].index, g[e].length,
...
int CompositeGraph::get_edge_index(NodeIndex u, NodeIndex v, double cost) const
{
...
EdgeID CompositeGraph::get_edge_id(NodeIndex u, NodeIndex v, double cost) const
{
