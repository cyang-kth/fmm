...
STMATCHConfig::STMATCHConfig(int k_arg, double r_arg, double gps_error_arg,
                             double vmax_arg, double factor_arg,
                             double reverse_tolerance_arg)
    : k(k_arg), radius(r_arg), gps_error(gps_error_arg), vmax(vmax_arg),
      factor(factor_arg), reverse_tolerance(reverse_tolerance_arg){};

void STMATCHConfig::print() const
{
    SPDLOG_INFO("STMATCHAlgorithmConfig");
    SPDLOG_INFO("k {} radius {} gps_error {} vmax {} factor {}", k, radius,
                gps_error, vmax, factor);
    SPDLOG_INFO("reverse_tolerance {}", reverse_tolerance);
};

STMATCHConfig
STMATCHConfig::load_from_xml(const boost::property_tree::ptree &xml_data)
{
...
STMATCHConfig STMATCHConfig::load_from_arg(const cxxopts::ParseResult &arg_data)
{
...
{
    options.add_options()("k,candidates", "Number of candidates",
...
PyMatchResult STMATCH::match_wkt(const std::string &wkt,
                                 const STMATCHConfig &config)
{
    ...
    for (int i = 0; i < result.opt_candidate_path.size(); ++i) {
        ...
        output.candidates.push_back(
            {i, mc.c.edge->id, graph_.get_node_id(mc.c.edge->source),
             graph_.get_node_id(mc.c.edge->target), mc.c.dist, mc.c.offset,
             mc.c.edge->length, mc.ep, mc.tp, mc.sp_dist});
                                ...
                                const STMATCHConfig &config)
{
    ...
    Traj_Candidates tc =
        network_.search_tr_cs_knn(traj.geom, config.k, config.radius);
    ...
    return MatchResult{traj.id, matched_candidate_path, opath, cpath, indices,
                       mgeom};
}

std::string
STMATCH::match_gps_file(const FMM::CONFIG::GPSConfig &gps_config,
                        const FMM::CONFIG::ResultConfig &result_config,
                        const STMATCHConfig &stmatch_config, bool use_omp)
{
...
#pragma omp parallel for
            for (int i = 0; i < trajectories_fetched; ++i) {
                ...
                MM::MatchResult result = match_traj(trajectory, stmatch_config);
...
void STMATCH::update_tg(TransitionGraph *tg, const CompositeGraph &cg,
                        const Trajectory &traj, const STMATCHConfig &config)
{
                           ...
                           double delta)
{
    ...
    for (auto iter_a = la_ptr->begin(); iter_a != la_ptr->end(); ++iter_a) {
        ...
        std::transform(lb.begin(), lb.end(), targets.begin(),
                       [](TGNode &a) { return a.c->index; });
        std::vector<double> distances =
            shortest_path_upperbound(level, cg, source, targets, delta);
        for (auto iter_b = lb_ptr->begin(); iter_b != lb_ptr->end(); ++iter_b) {
            ...
            SPDLOG_TRACE(
                "L {} f {} t {} sp {} dist {} tp {} ep {} fcp {} tcp {}", level,
                iter_a->c->edge->id, iter_b->c->edge->id, distances[i], eu_dist,
                tp, iter_b->ep, iter_a->cumu_prob, temp);
                            ...
                            double reverse_tolerance)
{
    ...
    for (int i = 0; i < N - 1; ++i) {
        ...
        if ((a->edge->id != b->edge->id) ||
            ...
            auto segs =
                graph_.shortest_path_dijkstra(a->edge->target, b->edge->source);
            // No transition found
            if (segs.empty() && a->edge->target != b->edge->source) {
                SPDLOG_TRACE("Candidate {} has disconnected edge {} to {}", i,
                             a->edge->id, b->edge->id);
                indices->clear();
                return {};
            }
