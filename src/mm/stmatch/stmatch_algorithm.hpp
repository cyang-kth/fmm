...
struct STMATCHConfig
{
     ...
     * Load from xml data
     */
    static STMATCHConfig
    load_from_xml(const boost::property_tree::ptree &xml_data);
    /**
...
class STMATCH
{
  public:
     ...
     */
    PYTHON::PyMatchResult match_wkt(const std::string &wkt,
     ...
     * statistics.
     */
    std::string match_gps_file(const FMM::CONFIG::GPSConfig &gps_config,
  ...
  protected:
     ...
     */
    void update_tg(TransitionGraph *tg, const CompositeGraph &cg,
                   const CORE::Trajectory &traj, const STMATCHConfig &config);
    /**
    ...
    void update_layer(int level, TGLayer *la_ptr, TGLayer *lb_ptr,
                      const CompositeGraph &cg, double eu_dist, double delta);
    ...
    std::vector<double> shortest_path_upperbound(
        int level, const CompositeGraph &cg, NETWORK::NodeIndex source,
