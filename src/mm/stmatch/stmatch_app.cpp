...
void STMATCHApp::run()
{
    ...
    const STMATCHConfig &stmatch_config = config_.stmatch_config;
...
#pragma omp parallel for
            for (int i = 0; i < trajectories_fetched; ++i) {
                ...
                MM::MatchResult result =
                    mm_model.match_traj(trajectory, stmatch_config);
...
#pragma omp critical
    ...
    double time_spent_exclude_input =
        UTIL::get_duration(corrected_begin, end_time);
    SPDLOG_INFO("Time takes {}", time_spent);
    SPDLOG_INFO("Time takes excluding input {}", time_spent_exclude_input);
    SPDLOG_INFO("Finish map match total points {} matched {}", total_points,
                points_matched);
    SPDLOG_INFO("Matched percentage: {}",
