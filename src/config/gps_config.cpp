    ...
    const boost::property_tree::ptree &xml_data)
{
    ...
    config.timestamp = xml_data.get("config.input.gps.timestamp", "timestamp");
    config.x = xml_data.get("config.input.gps.x", "x");
    config.y = xml_data.get("config.input.gps.y", "y");
    config.gps_point =
        !(!xml_data.get_child_optional("config.input.gps.gps_point"));
    return config;
};

FMM::CONFIG::GPSConfig
FMM::CONFIG::GPSConfig::load_from_arg(const cxxopts::ParseResult &arg_data)
{
    GPSConfig config;
...
{
    options.add_options()("gps", "GPS file name",
...
{
    std::string fn_extension = file.substr(file.find_last_of(".") + 1);
