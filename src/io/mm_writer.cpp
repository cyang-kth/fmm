...
void CSVMatchResultWriter::write_result(const FMM::CORE::Trajectory &traj,
                                        const FMM::MM::MatchResult &result)
{
    ...
    if (config_.write_pgeom) {
        ...
        if (!result.opt_candidate_path.empty()) {
            ...
            for (int i = 0; i < N; ++i) {
                const FMM::CORE::Point &point =
                    result.opt_candidate_path[i].c.point;
    ...
    if (config_.write_speed) {
        ...
        if (!result.opt_candidate_path.empty() && !traj.timestamps.empty()) {
            ...
            for (int i = 1; i < N; ++i) {
                ...
                buf << (duration > 0
                            ? (result.opt_candidate_path[i].sp_dist / duration)
                            : 0)
                    << (i == N - 1 ? "" : ",");
            }
        }
