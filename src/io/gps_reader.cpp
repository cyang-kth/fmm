                                           ...
                                           const std::string &timestamp_name)
{
    ...
    OGRRegisterAll();
    poDS = (GDALDataset *)GDALOpenEx(filename.c_str(), GDAL_OF_VECTOR, NULL,
                                     NULL, NULL);
    if (poDS == NULL) {
    ...
    if (id_idx < 0) {
        std::string message =
            (boost::format("Id column %1% not found") % id_name).str();
        SPDLOG_CRITICAL(message);
        GDALClose(poDS);
        throw std::runtime_error(message);
    }
    ...
    if (wkbFlatten(ogrFDefn->GetGeomType()) != wkbLineString) {
        std::string message =
            (boost::format("Geometry type is %1%, which should be linestring") %
...
bool GDALTrajectoryReader::has_timestamp() { return timestamp_idx > 0; }
...
int GDALTrajectoryReader::get_num_trajectories() { return NUM_FEATURES; }

void GDALTrajectoryReader::close() { GDALClose(poDS); }
    ...
    : ifs(e_filename)
{
    ...
    if (id_idx < 0 || geom_idx < 0) {
        std::string message =
            (boost::format("Id %1% or Geometry column %2% not found") %
             id_name % geom_name)
                .str();
        SPDLOG_CRITICAL(message);
        throw std::runtime_error(message);
    ...
    SPDLOG_INFO("Id index {} Geometry index {} Timstamp index {}", id_idx,
                geom_idx, timestamp_idx);
}

std::vector<double> CSVTrajectoryReader::string2time(const std::string &str)
{
...
bool CSVTrajectoryReader::has_timestamp() { return timestamp_idx > 0; }
...
bool CSVTrajectoryReader::has_next_trajectory() { return ifs.peek() != EOF; }
...
void CSVTrajectoryReader::close() { ifs.close(); }
    ...
    : ifs(e_filename)
{
    ...
    if (id_idx < 0 || x_idx < 0 || y_idx < 0) {
        ...
        if (x_idx < 0) {
            std::string message =
                (boost::format("X column name %1% not found") % x_name).str();
            SPDLOG_CRITICAL(message);
            throw std::runtime_error(message);
        }
        if (y_idx < 0) {
            std::string message =
                (boost::format("Y column name %1% not found") % y_name).str();
            SPDLOG_CRITICAL(message);
            throw std::runtime_error(message);
        }
    }
    ...
    SPDLOG_INFO("Id index {} x index {} y index {} time index {}", id_idx,
                x_idx, y_idx, timestamp_idx);
}
...
bool CSVPointReader::has_next_trajectory() { return ifs.peek() != EOF; }
...
void CSVPointReader::close() { ifs.close(); }

bool CSVPointReader::has_timestamp() { return timestamp_idx > 0; }
...
GPSReader::GPSReader(const FMM::CONFIG::GPSConfig &config)
{
    ...
    if (mode == 0) {
        ...
        reader = std::make_shared<GDALTrajectoryReader>(config.file, config.id,
                                                        config.timestamp);
    } else if (mode == 1) {
