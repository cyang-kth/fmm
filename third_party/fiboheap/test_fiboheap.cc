...
struct lowerI
{
    bool operator()(const int &d1, const int &d2) { return d1 > d2; }
};
...
int main(int argc, char *argv[])
{
    ...
    fill_heaps(fh, pqueue, n);
    // std::cerr << "top pqueue: " << pqueue.top() << " -- top fh: " << fh.top()
    // << std::endl;
    int r = pqueue.top() - 1;
    // std::cerr << "old val: " << pqueue.top() << " -- new val: " << r <<
    // std::endl;
    pqueue.pop();
    pqueue.push(r);
    std::make_heap(const_cast<int *>(&pqueue.top()),
                   const_cast<int *>(&pqueue.top()) + pqueue.size(), lowerI());
