...
class fp
{
  ...
  public:
    ...
    template <typename Double> explicit fp(Double d)
    {
        ...
        e = static_cast<int>(biased_e - exponent_bias -
    ...
    {
        lower = f == implicit_bit ? fp((f << 2) - 1, e - 2)
                                  : fp((f << 1) - 1, e - 1);
        upper = fp((f << 1) + 1, e - 1);
...
grisu2_format(Double value, buffer &buf, core_format_specs specs)
{
    ...
    grisu2_prettify(params, size, cached_exp + exp,
                    prettify_handler(buf, size));
    return true;
}
...
FMT_FUNC internal::utf8_to_utf16::utf8_to_utf16(string_view s)
{
    ...
    length = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, s.data(),
                                 s_size, &buffer_[0], length);
    if (length == 0)
        FMT_THROW(windows_error(GetLastError(), ERROR_MSG));
    buffer_[length] = 0;
}
